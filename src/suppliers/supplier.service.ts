  ...
  async updateById(id: number, updateSupplierDto: UpdateSupplierDto) {
    ...
    if (conflictSupplier) {
      if (conflictSupplier.phone === updateSupplierDto.phone) {
        throw new ConflictException(`Phone number ${updateSupplierDto.phone} already exists`);
      }
      if (conflictSupplier.email === updateSupplierDto.email) {
        throw new ConflictException(`Email ${updateSupplierDto.email} already exists`);
      }
    }
