    ...
    try {
      ...
      if (vnpAmount !== payment.totalAmount) {
        ...
        return {
          ...
          payment,
          message: 'Số tiền thanh toán không hợp lệ',
          redirectUrl: `${this.configService.get<string>('paymentResultRedirectPrefixUrl')}payment-failed?message=Số tiền thanh toán không hợp lệ`,
        };
      }
        ...
        return {
          ...
          message:
            payment.status === PaymentStatus.COMPLETED
              ? 'Thanh toán đã được xử lý trước đó'
              : 'Thanh toán đã bị hủy trước đó',
          redirectUrl: `${this.configService.get<string>('paymentResultRedirectPrefixUrl')}${
            payment.status === PaymentStatus.COMPLETED ? 'payment-success' : 'payment-failed'
          }`,
        };
      }
      ...
      return {
        ...
        message: verifyResult.isSuccess
          ...
          : verifyResult.message || 'Thanh toán thất bại',
        redirectUrl: `${this.configService.get<string>('paymentResultRedirectPrefixUrl')}${
          verifyResult.isSuccess ? 'payment-success' : 'payment-failed'
        }`,
      };
    } catch (error: any) {
      this.logger.error(
      ...
      return {
        success: false,
        message: `Lỗi xử lý thanh toán: ${error.message}`,
        redirectUrl: `${this.configService.get<string>('paymentResultRedirectPrefixUrl')}payment-failed?message=${encodeURIComponent(error.message)}`,
      };
    }
  }
